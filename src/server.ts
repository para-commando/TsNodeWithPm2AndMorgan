...
app.on('urlencodedMiddleWareFailed', function (message:String) {
  console.log('The myCustomEvent event was emitted with the message: ' + message);
});
...
console.log("🚀 ~ file: server.ts:147 ~ app.get ~ req:", req.host)
console.log("🚀 ~ file: server.ts:199 ~ app.get ~ req.params.id:", req.params.id)
console.log("🚀 ~ file: server.ts:172 ~ req.baseUrl:", req.baseUrl)
console.log("🚀 ~ file: server.ts:228 ~ app.use ~ req.originalUrl:", req.originalUrl)
...
app.get('/res.links', (req:any, res:any) => {
  const data = {
    users: [
      {
        name: 'Bard',
